...
const listen = (msg, bot) => {
  ...
  const apiaiReq = apiai.textRequest(msg.content, {
    sessionId: "bucksy",
  });
  ...
  apiaiReq.on("response", async (response) => {
    ...
    const pokemon =
      ...
      typeof response.result.parameters.pokemon !== "undefined"
        ? response.result.parameters.pokemon
            .toLowerCase()
            .replace(".", "-")
            .replace(" ", "")
            .replace("'", "")
    ...
    const specs =
      response.result.parameters &&
      typeof response.result.parameters.specs !== "undefined"
        ? response.result.parameters.specs
        : "";
    const get_type_effectiveness =
      response.result.parameters &&
      response.result.parameters.type_effectiveness
        ? true
        : false;
    const get_joke_intent =
      response.result.metadata.intentName === "joke" ? true : false;
      ...
      for (const spec of specs) {
        if (pokemon_endpoint.indexOf(spec) !== -1) {
          ...
          switch (spec) {
            ...
            case "shiny":
              await database
                .shinyExists(pokemon.toLowerCase())
    ...
    if (get_type_effectiveness) {
      ...
      if (typeof type_effectiveness !== "undefined") {
        ...
        const type_effectiveness_word =
          response.result.contexts[0].parameters["type_effectiveness.original"];
        const pokemon_type_comes_first =
          response.result.resolvedQuery.indexOf(pokemon_type) <
          response.result.resolvedQuery.indexOf(type_effectiveness_word)
            ? true
            : false;
        const exempt_words = [
          "resistant",
        ...
        if (type_effectiveness === "effective against") {
          if (pokemon_type_comes_first) {
            ...
            type_effectiveness_formatted = "double_damage_to".replace(
              /_/g,
              " "
            );
            from_or_to = "to";
            ...
            type_effectiveness_formatted = "double_damage_from".replace(
              /_/g,
              " "
            );
            from_or_to = "from";
        ...
        const damage_relations =
          output.data.damage_relations[type_effectiveness].length > 0
            ? output.data.damage_relations[type_effectiveness]
                .map((item) => item.name)
                .join(", ")
            : "none";
        const nature_of_damage =
          from_or_to === "from" ? "receives" : "inflicts";
    ...
    if (response_obj.fulfillmentText && !response_obj.payload) {
      ...
      bot.log.debug("ai attempting to send image");
      const file = response_obj.payload.url;
      const photoEmbed = {
        title: pokemon,
      };
      msg.channel.send({ files: [file], embed: photoEmbed });
    } else {
