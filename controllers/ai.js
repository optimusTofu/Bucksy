...
const listen = (msg) => {
    ...
    apiaiReq.on('response', async(response) => {
        ...
        const pokemon = (response.result.parameters && typeof response.result.parameters.pokemon !== "undefined") ? response.result.parameters.pokemon.toLowerCase().replace('.', '-').replace(' ', '').replace("'", "") : '';
        const specs = (response.result.parameters && typeof response.result.parameters.specs !== "undefined") ? response.result.parameters.specs : '';
        const get_type_effectiveness = (response.result.parameters && response.result.parameters.type_effectiveness) ? true : false;
        ...
        if (response_obj.fulfillmentText && !response_obj.payload) {
            ...
            console.debug('ai attempting to send image');
